...
class SuperconductivityDataset(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        data_dir = os.path.join(dl_manager.download_and_extract(urls), "superconductivity_dataset")
    ...
    def _generate_examples(self, datapath, jsonlpath, split):
        ...
        with open(jsonlpath, encoding="utf-8") as f:
            for key, row in enumerate(f):
                data = json.loads(row)
                paper_text = open(os.path.join(datapath, data["doi"].split("/")[1], 'text.txt'), 'r').read()

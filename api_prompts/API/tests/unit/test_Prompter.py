...
class Test_Prompter:
    ...
    def test_query(self):
        ...
        results = jsonl_read(f"{root}/tests/unit/help_files/results/test_data_1/results.jsonl")
        state = False
        key_list = ["paper", "question", "logs", "num", "chain", "prompt", "requests", "answer", "response", "LLM", "rouge2", "rougeL", "sentence_bool", "confidence", "sentence", "E2E"]
        if len(results) == 0:
            assert state
        else:
            for i in results[0].keys():
                ...
                if i == "logs":
                    ...
                    for j in results[0][i][0].keys():
                        if j not in key_list:
                            assert state
                        if j == "requests":
                            for k in results[0][i][0][j][0].keys():
